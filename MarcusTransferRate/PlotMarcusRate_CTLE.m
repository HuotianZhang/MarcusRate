...
field_name = 'kCTLE0515';
num_curve = 3;
deltaG_values = 0.00:0.1:(-0.05+num_curve*0.1);

data = kCTLE_stark_vars.(field_name);
...
for i = 1:length(deltaG_values)
    ...
    legend_entries{i} = sprintf('\\Delta{\\itE}_{CT-Ex} = %.2f (demo)', deltaG_values(i));
    else
    legend_entries{i} = sprintf('\\Delta{\\itE}_{CT-Ex} = %.2f', deltaG_values(i));
    end
end
